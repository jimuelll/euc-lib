...
const respond = (handler) => async (req, res) => { try { const result = await handler(req); if (req.method !== "GET" && result && typeof result === "object") res.locals.auditEnqueued = true; res.json(result); } catch (error) { logError("[clearance]", error); res.status(error.status || 500).json({ message: error.message || "Clearance request failed", clearance: error.clearance }); } };
