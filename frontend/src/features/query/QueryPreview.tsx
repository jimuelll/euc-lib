...
const QueryPreview = () => {
  ...
  return <main className="min-h-screen bg-background px-4 py-6 text-foreground sm:px-8 lg:px-12">
    <div className="mx-auto max-w-[1600px] space-y-5">
      ...
      {loading ? <div className="flex min-h-64 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Preparing spreadsheet preview…</div> : result && <section className="admin-panel-surface admin-etched-border overflow-hidden border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b border-border bg-muted/30">{result.columns.map((column) => <th key={column.key} scope="col" className="whitespace-nowrap px-4 py-3 text-xs font-semibold text-muted-foreground">{column.label}</th>)}</tr></thead><tbody className="divide-y divide-border">{result.rows.map((row, index) => <tr key={index}>{result.columns.map((column) => <td key={column.key} className="max-w-[26rem] px-4 py-3 align-top"><span className="block break-words">{formatQueryValue(row[column.key], column.type)}</span></td>)}</tr>)}</tbody></table></div>{!result.rows.length && <p className="px-5 py-12 text-center text-sm text-muted-foreground">No records match this query.</p>}</section>}
    </div>
  </main>;
};
