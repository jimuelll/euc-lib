...
export default function ReportWorkspace() {
  ...
  return <div className="space-y-5">
    ...
    <form onSubmit={run} className="border-y border-border bg-muted/20 px-3 py-4 sm:px-5"><div className="grid gap-3 md:grid-cols-[minmax(15rem,0.8fr)_minmax(14rem,1fr)_auto] md:items-end">
      ...
      <div className="space-y-1.5"><Label htmlFor="report-search" className="text-xs font-medium text-muted-foreground">Search</Label><div className="relative"><Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input id="report-search" value={String(draft.search ?? "")} onChange={(event) => update("search", event.target.value)} className="h-11 rounded-md bg-background pl-10" placeholder="Search this report…" /></div></div>
      <Button type="submit" className="h-11 rounded-md"><Search className="mr-2 h-4 w-4" />Run report</Button></div>
      <div className="mt-4 grid gap-3 border-t border-border/70 pt-4 sm:grid-cols-2 lg:grid-cols-4"><div className="space-y-1.5"><Label htmlFor="report-date-from" className="text-xs font-medium text-muted-foreground">Start date</Label><Input id="report-date-from" type="date" value={String(draft.dateFrom ?? "")} onChange={(event) => update("dateFrom", event.target.value)} className="h-11 rounded-md bg-background" /></div><div className="space-y-1.5"><Label htmlFor="report-date-to" className="text-xs font-medium text-muted-foreground">End date</Label><Input id="report-date-to" type="date" value={String(draft.dateTo ?? "")} onChange={(event) => update("dateTo", event.target.value)} className="h-11 rounded-md bg-background" /></div>{selected.ranking && <div className="space-y-1.5"><Label htmlFor="report-ranking" className="text-xs font-medium text-muted-foreground">Rank users by</Label><Select value={String(draft.ranking)} onValueChange={(value) => update("ranking", value)}><SelectTrigger id="report-ranking" className="h-11 rounded-md bg-background"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="borrowings">Most borrowings</SelectItem><SelectItem value="checkins">Most library check-ins</SelectItem></SelectContent></Select></div>}<div className="flex items-end justify-end"><Button type="button" variant="ghost" className="rounded-md" onClick={reset}>Reset filters</Button></div></div><p className="mt-3 text-xs text-muted-foreground">{selected.description}</p></form>
