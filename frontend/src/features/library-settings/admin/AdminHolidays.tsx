...
import {
  ...
  deleteAcademicProgram, deleteAcademicTerm, deleteLibraryHoliday, restoreAcademicProgram, restoreLibraryHoliday,
...
} from "@/features/library-settings/admin/api";
...
export default function AcademicSettings() {
  ...
  const removeProgram = async (item: AcademicProgram) => {
    const users = Number(item.user_reference_count || 0);
    const holdings = Number(item.holding_reference_count || 0);
    const references = users + holdings;
    const explanation = references
      ? ` It is used by ${users} user record${users === 1 ? "" : "s"} and ${holdings} holding${holdings === 1 ? "" : "s"}, so it will be archived and those records will keep the reference.`
      : " It has no user or holding references, so it will be permanently deleted.";
  ...
  const restoreProgram = (item: AcademicProgram) => run(`program-restore-${item.id}`, () => restoreAcademicProgram(item.id), "Program / course restored");
  const archiveHoliday = async (item: LibraryHoliday) => {
    if (!window.confirm(`Archive “${item.name}”? Holidays are retained because saved loan due dates do not show whether this date was used.`)) return;
    await run(`holiday-archive-${item.id}`, () => deleteLibraryHoliday(item.id), "Holiday archived");
  };
  const restoreHoliday = (item: LibraryHoliday) => run(`holiday-restore-${item.id}`, () => restoreLibraryHoliday(item.id), "Holiday restored");
  const statusSwitch = (value: "active" | "archived", onChange: (value: "active" | "archived") => void, label: string) => <div className="flex gap-1 border border-border p-1" role="group" aria-label={`${label} status`}>{(["active", "archived"] as const).map((option) => <Button key={option} type="button" size="sm" variant={value === option ? "secondary" : "ghost"} aria-pressed={value === option} onClick={() => onChange(option)}>{option === "active" ? "Active" : "Archived"}</Button>)}</div>;
  ...
  return <AdminPage title="Academic calendar & settings" description="Manage terms, standardized programs, and no-service dates." contentWidth="wide"><Tabs value={tab} onValueChange={setTab}>
    <div className="border-y border-border bg-muted/20 px-2 py-2 sm:px-3"><TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-md bg-transparent p-0"><TabsTrigger value="terms" className="gap-2 rounded-sm px-3 py-2 text-xs"><CalendarDays className="h-4 w-4" />Academic terms</TabsTrigger><TabsTrigger value="programs" className="gap-2 rounded-sm px-3 py-2 text-xs"><BookOpen className="h-4 w-4" />Programs / Courses</TabsTrigger><TabsTrigger value="holidays" className="gap-2 rounded-sm px-3 py-2 text-xs"><CalendarOff className="h-4 w-4" />Holidays</TabsTrigger></TabsList></div>
    ...
    <TabsContent value="programs" className="mt-0"><Header icon={BookOpen} title="Programs / Courses" detail="Active choices are used for new accounts and holdings. Referenced entries can be restored from the Archived view." /><div className="mt-5 grid gap-5 lg:grid-cols-2"><AdminPanel title={editingProgramId ? "Edit program / course" : "Add program / course"}><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void run("program", () => editingProgramId ? updateAcademicProgram(editingProgramId, program) : createAcademicProgram(program), editingProgramId ? "Program / course updated" : "Program / course added").then((ok) => ok && cancelProgram()); }}><div><Label htmlFor="program-name">Program / course name</Label><Input id="program-name" className="mt-2" value={program} onChange={(event) => setProgram(event.target.value)} placeholder="BS Information Technology" required /></div><div className="flex gap-2"><BusyButton type="submit" pending={pending === "program"}>{editingProgramId ? "Save program" : "Add program / course"}</BusyButton>{editingProgramId && <Button type="button" variant="outline" onClick={cancelProgram} disabled={Boolean(pending)}>Cancel</Button>}</div></form></AdminPanel><AdminPanel title="Configured programs / courses" contentClassName="p-0"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3"> <p className="text-sm text-muted-foreground">Counts include archived user accounts and holdings.</p>{statusSwitch(programStatus, setProgramStatus, "Program / course")}</div>{rows(programs.filter((item) => programStatus === "active" ? Boolean(item.is_active) : !item.is_active), `No ${programStatus} programs / courses.`, (item: AcademicProgram) => { const users = Number(item.user_reference_count || 0); const holdings = Number(item.holding_reference_count || 0); return <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div className="min-w-0"><p className="font-medium">{item.name}</p><p className="mt-1 text-sm text-muted-foreground">{users} user reference{users === 1 ? "" : "s"} · {holdings} holding reference{holdings === 1 ? "" : "s"}</p></div><div className="flex shrink-0 gap-2">{item.is_active ? <><Button type="button" size="sm" variant="outline" onClick={() => { setEditingProgramId(item.id); setProgram(item.name); }} disabled={Boolean(pending)}><Pencil className="mr-1.5 h-3.5 w-3.5" />Edit</Button><Button type="button" size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => void removeProgram(item)} disabled={Boolean(pending)}>{users + holdings ? <Archive className="mr-1.5 h-3.5 w-3.5" /> : <Trash2 className="mr-1.5 h-3.5 w-3.5" />}{users + holdings ? "Archive" : "Delete"}</Button></> : <Button type="button" size="sm" variant="outline" onClick={() => void restoreProgram(item)} disabled={Boolean(pending)}><RotateCcw className="mr-1.5 h-3.5 w-3.5" />Restore</Button>}</div></div>; })}</AdminPanel></div></TabsContent>
    <TabsContent value="holidays" className="mt-0"><Header icon={CalendarOff} title="Holidays" detail="Active holidays extend new loan due dates. Restoring or archiving a holiday does not recalculate existing due dates." /><div className="mt-5 grid gap-5 lg:grid-cols-2"><AdminPanel title={editingHolidayId ? "Edit holiday" : "Add holiday"}><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void run("holiday", () => editingHolidayId ? updateLibraryHoliday(editingHolidayId, holiday) : createLibraryHoliday(holiday), editingHolidayId ? "Holiday updated" : "Holiday added").then((ok) => ok && cancelHoliday()); }}><div><Label htmlFor="holiday-name">Holiday name</Label><Input id="holiday-name" className="mt-2" value={holiday.name} onChange={(event) => setHoliday({ ...holiday, name: event.target.value })} placeholder="Holiday name" required /></div><div><Label htmlFor="holiday-date">Date</Label><Input id="holiday-date" className="mt-2" type="date" value={holiday.holiday_date} onChange={(event) => setHoliday({ ...holiday, holiday_date: event.target.value })} required /></div><div><Label htmlFor="holiday-note">Note <span className="text-muted-foreground">(optional)</span></Label><Input id="holiday-note" className="mt-2" value={holiday.description} onChange={(event) => setHoliday({ ...holiday, description: event.target.value })} placeholder="Optional note" /></div><div className="flex gap-2"><BusyButton type="submit" pending={pending === "holiday"}>{editingHolidayId ? "Save holiday" : "Add holiday"}</BusyButton>{editingHolidayId && <Button type="button" variant="outline" onClick={cancelHoliday} disabled={Boolean(pending)}>Cancel</Button>}</div></form></AdminPanel><AdminPanel title="Configured holidays" contentClassName="p-0"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3"><p className="max-w-lg text-sm text-muted-foreground">Usage cannot be proved from stored due dates, so holidays are archived and kept restorable.</p>{statusSwitch(holidayStatus, setHolidayStatus, "Holiday")}</div>{rows(holidays.filter((item) => holidayStatus === "active" ? Boolean(item.is_active) : !item.is_active), `No ${holidayStatus} holidays.`, (item: LibraryHoliday) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4"><div className="min-w-0"><p className="font-medium">{item.name}</p><p className="mt-1 text-sm text-muted-foreground">{fmt(item.holiday_date)}{item.description ? ` · ${item.description}` : ""}</p></div><div className="flex shrink-0 gap-2">{item.is_active ? <><Button type="button" size="sm" variant="outline" onClick={() => { setEditingHolidayId(item.id); setHoliday({ name: item.name, holiday_date: item.holiday_date.slice(0, 10), description: item.description ?? "" }); }} disabled={Boolean(pending)}><Pencil className="mr-1.5 h-3.5 w-3.5" />Edit</Button><Button type="button" size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => void archiveHoliday(item)} disabled={Boolean(pending)}><Archive className="mr-1.5 h-3.5 w-3.5" />Archive</Button></> : <Button type="button" size="sm" variant="outline" onClick={() => void restoreHoliday(item)} disabled={Boolean(pending)}><RotateCcw className="mr-1.5 h-3.5 w-3.5" />Restore</Button>}</div></div>)}</AdminPanel></div></TabsContent>
  </Tabs><AdminDepartments /></AdminPage>;
}
