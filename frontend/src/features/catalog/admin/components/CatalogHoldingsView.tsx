...
export default function CatalogHoldingsView({ onBack, onSelect }: Props) {
  ...
  return <section className="space-y-4" aria-labelledby="holdings-view-heading">
    ...
    <div className="overflow-hidden border border-border bg-card">
      {loading ? <div className="flex items-center justify-center gap-2 p-12 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading holdings…</div> : result.rows.length === 0 ? <div className="px-5 py-14 text-center"><BookOpen className="mx-auto h-8 w-8 text-muted-foreground/40" /><p className="mt-3 text-sm font-medium text-foreground">{error || "No holdings match these filters"}</p><p className="mt-1 text-sm text-muted-foreground">{error ? "" : "Try another search or show copies that need an accession number."}</p></div> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm">
