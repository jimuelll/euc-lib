...
import { fetchLibrarySettings, updateLibrarySettings } from "@/features/library-settings";
import { fetchPaymentOverview, fetchUserPaymentOverview, settleUserPayment, type PaymentOverviewResponse, type PaymentRow, type UserPaymentOverviewResponse } from "@/features/clearance/api";
...
const AdminPayment = () => {
  ...
  const loadOverview = async (mode: "initial" | "refresh" = "initial", page = 1) => {
    ...
    try {
      const [settingsPayload, overviewPayload] = await Promise.all([fetchLibrarySettings(), fetchPaymentOverview(page)]);

      setHourlyFine(String(settingsPayload.settings.overdue_fine_per_hour ?? 0));
      setOverview(overviewPayload);
      setPaymentPage(overviewPayload.pagination ?? { page, limit: 20, total: overviewPayload.summary.total_records, totalPages: 1 });
    } catch (error: any) {
  ...
  const handleLookupUser = async (event?: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await fetchUserPaymentOverview(trimmedId);

      setUserOverview(result);
      setSettlementAmount(result.summary.total_unsettled_amount ? String(result.summary.total_unsettled_amount) : "");
  ...
  const handleSettlePayments = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await settleUserPayment(userOverview.user.student_employee_id, numericAmount);
